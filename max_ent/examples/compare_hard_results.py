...
def draw_diagram(scobee, our, y_label, lens, thresholds, draw_scobee=True):
    plt.figure()
    if draw_scobee:
...
def main():
    ...
    s_fp, s_kl = get_stats(true_cons, scobee_cons, true_demo, scobee_demo, n_set, n_len)
    ...
    for i, t in enumerate(thresholds):
        ...
        o_fp[i], o_kl[i] = get_stats(true_cons, our_cons, true_demo, our_hard_demo, n_set, n_len)


    draw_diagram(s_fp, o_fp, 'False Positive Rate', lens, thresholds, draw_scobee=False)
    plt.savefig('./reports/hard/hard_all_fp.pdf')
    draw_diagram(s_kl, o_kl, 'KL-Divergence', lens, thresholds, draw_scobee=False)
    plt.savefig('./reports/hard/hard_all_kl.pdf')
    
    draw_diagram(s_fp, o_fp, 'False Positive Rate', lens, [0.6], draw_scobee=True)
    plt.savefig('./reports/hard/hard_best_fp.pdf')
    draw_diagram(s_kl, o_kl, 'KL-Divergence', lens, [0.6], draw_scobee=True)

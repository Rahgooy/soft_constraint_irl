...
def create_world(title, blue, green, cs=[], ca=[], cc=[], start=0, goal=8, vmin=-50, vmax=10, check=False):
    ...
    if check and not demo: return None,None,None,None #CHECK WHETHER START AND GOAL ARE REACHABLE
    vmin = -50
    vmax = 10
...
def count_states(trajectories, grid, nominal, constraints):
    ...
    for trajectory in trajectories:
        ...
        reward, reward_n, count_cs, count_ca, count_cb, count_cg = total_reward(trajectory, grid, nominal, constraints)
        avg_reward += reward
...
def plot_statistics(df, learned_matrix, nominal_matrix, denominator, save_path, label = "avg_norm_length", label_nominal = "avg_length", n_tests=100):
    ...
    constrained_avg_norm_length= np.mean([learned_matrix[i][i][label_nominal]/denominator for i in range(n_tests)])
    nominal_avg_norm_length= np.mean([nominal_matrix[i][i][label_nominal]/denominator for i in range(n_tests)])
...
def compute_statistics(nominal_matrix, constrained_matrix, learned_matrix, mdft_matrix, worlds, n_tests, i, type_m):
    ...
    for test in range(n_tests):
        ...
        avg_norm_length_mdft.append(mdft_matrix[test][i][i]['avg_length']/min_nominal_length)#nominal_matrix[test][test]['avg_length'])
        

        avg_rew_mdft.append(mdft_matrix[test][i][i]['avg_reward'])
       
        avg_norm_rew_mdft.append(mdft_matrix[test][i][i]['avg_reward']/learned_matrix[test][test]['avg_reward'])

        avg_vc_mdft.append(mdft_matrix[test][i][i]['avg_violated'])

        avg_norm_vc_mdft.append(mdft_matrix[test][i][i]['avg_violated']/learned_matrix[test][test]['avg_violated'])
